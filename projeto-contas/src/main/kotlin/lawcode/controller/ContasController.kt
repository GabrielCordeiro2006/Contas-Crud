package lawcode.controller
import lawcode.dto.ContasResposta
import lawcode.entity.Contas
import jakarta.validation.Valid
import org.springframework.http.ResponseEntity
import org.springframework.web.bind.annotation.*
import lawcode.repository.ContasReposity
...
class ContasController(val repositorio: ContasReposity) {
    ...
    @GetMapping("/filtro")
    fun getByDataVencimento(@RequestParam(required = false) dataVencimento: LocalDate?): ResponseEntity<List<Contas>> {
        val filtroData = dataVencimento ?: LocalDate.now() // Usa a data fornecida ou a data atual se não houver parâmetro
        val contas = repositorio.findByDataVencimentoGreaterThanEqualOrderByDataVencimentoAsc(filtroData)
        return if (contas.isEmpty()) {
            ResponseEntity.status(204).build()
        } else {
            ResponseEntity.status(200).body(contas)
